...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="flex justify-between items-center mb-8">
        ...
        <a href="{{ route('admin.export.orders') }}" class="btn-primary px-4 py-2">
            Exporter CSV
        </a>
    </div>
    ...
    <div class="card p-6 mb-8">
        <form method="GET" action="{{ route('admin.orders.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div>
                ...
                <input type="text" name="client" placeholder="email@exemple.com" value="{{ request('client') }}"
                       class="w-full rounded-lg border-gray-600 bg-gray-800 text-white">
            </div>
            <div class="md:col-span-4 flex justify-end">
                <button type="submit" class="btn-primary px-6 py-2">Filtrer</button>
                @if(request()->anyFilled(['status', 'from', 'to', 'client']))
                    <a href="{{ route('admin.orders.index') }}" class="btn-outline ml-3 px-6 py-2">Réinitialiser</a>
                @endif
            </div>
        </form>
    </div>
            ...
            <table class="min-w-full divide-y divide-gray-700">
                ...
                <tbody class="bg-gray-900 divide-y divide-gray-800">
                    @forelse($orders as $order)
                        <tr class="hover:bg-gray-800/50 transition-colors" id="order-row-{{ $order->id }}">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-white">{{ $order->id }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-white">{{ $order->user->email }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-white">{{ number_format($order->total, 2) }} $</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <form action="{{ route('admin.orders.updateStatus', $order) }}" method="POST" class="order-status-form" data-order-id="{{ $order->id }}">
                                    @csrf @method('PATCH')
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                <span class="px-2 py-1 text-xs rounded-full status-badge
                                    @if($order->payment_status == 'pending') bg-yellow-900/30 text-yellow-300
                                    @elseif($order->payment_status == 'paid') bg-green-900/30 text-green-300
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right">
                                <a href="{{ route('orders.show', $order) }}" class="text-blue-400 hover:text-blue-300 transition">Voir</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
