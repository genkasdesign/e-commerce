...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="flex justify-between items-center mb-8">
        ...
        <p class="text-sm text-gray-400">
            {{ isset($cart) && count($cart) > 0 ? count($cart) . ' article(s)' : '0 article' }}
        </p>
    </div>
    ...
    @if(empty($cart))
        <div class="card p-12 text-center" data-aos="fade-up">
            ...
            <a href="{{ route('dashboard') }}" class="btn-primary inline-block mt-6 px-6 py-3">Voir les produits</a>
        </div>
    @else
        <div class="card overflow-hidden" data-aos="fade-up">
                ...
                <table class="min-w-full divide-y divide-gray-700">
                    ...
                    <tbody class="bg-gray-900 divide-y divide-gray-800" id="cart-items">
                        ...
                        @php
                            ...
                            $grandTotal += $subtotal;
                        @endphp
                        <tr class="hover:bg-gray-800/50 transition-colors" data-product-id="{{ $id }}">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center space-x-4">
                                    ...
                                    <div>
                                        ...
                                        <div class="text-xs text-gray-400">Ref: #{{ $id }}</div>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <form action="{{ route('cart.update', $id) }}" method="POST" class="cart-update-form" data-product-id="{{ $id }}">
                                    @csrf @method('PATCH')
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right">
                                <form action="{{ route('cart.remove', $id) }}" method="POST" class="inline-block cart-remove-form">
                                    @csrf @method('DELETE')
            ...
            <div class="bg-gray-800/50 px-6 py-4 border-t border-gray-700 flex flex-col sm:flex-row justify-between items-center gap-4">
                <div class="text-sm text-gray-400">
                    <span class="font-medium text-white">{{ count($cart) }} article(s)</span> dans votre panier
                </div>
